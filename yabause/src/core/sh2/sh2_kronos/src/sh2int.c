...
{
  LOCK(context);
  /* SH2EvaluateInterrupt latches intPriority and intVector when a source
   * asserts, checking the mask at that moment; delivery happens later, and
   * software may have raised SR.I in between. The hardware re-evaluates the
   * mask at every instruction boundary and holds the request instead.
   *
   * Put the request back exactly where it was - the external line re-asserted,
   * the latch cleared - so nothing is lost and nothing spins: SH2EvaluateInterrupt
   * will not latch it again while the mask blocks it, and LDC Rm,SR, LDC.L
   * @Rm+,SR and RTE all call it once the mask drops. NMI (vector 0x0B) is
   * always accepted, as on hardware. */
