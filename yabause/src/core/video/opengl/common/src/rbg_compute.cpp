...
"      ky = kval;\n"
// ST-058-R2 6.5 : en mode 3 la donnee du coef n'est pas un facteur d'echelle
// mais la coordonnee Xp du point de vue, rechargee a chaque dot/ligne.
// Format 2 octets : entier signe 15 bits, 2 bits de partie fractionnaire.
"    } else if (para[paramid].coefmode == 3) {\n"
"      if((kdata&0x4000u)!=0u) Xp=float( int(kdata&0x7FFFu)| int(0xFFFFC000u) )/4.0;\n"
"      else Xp=float(kdata&0x7FFFu)/4.0;\n"
"    } \n"
"  }else{\n"
//powerslave
...
"    } else if (para[paramid].coefmode == 2) {\n"
"      ky = kval;\n"
// Format 4 octets : entier signe 24 bits, 8 bits de partie fractionnaire.
"    } else if (para[paramid].coefmode == 3) {\n"
"      if((kdata&0x00800000u)!=0u) Xp=float( int(kdata&0x00FFFFFFu)| int(0xFF800000u) )/256.0;\n"
"      else Xp=float(kdata&0x00FFFFFFu)/256.0;\n"
"    } \n"
"  }\n"
"  return 0;\n"
" }\n"
...
"    if (GetKValue(0, pos, ky, kx, Xp, lineaddr) == -1) { \n"
...
"        if (GetKValue(1, pos, ky, kx, Xp, lineaddr) == -1) { \n"
...
"  float fh = floor(kx * (Xsp + para[paramid].dx * original_pos.x) + Xp);\n"
